...
* A rule can be narrower than it sounds. `ActuatorExposureRule` (SCG001)
  used to fire only on a *wildcard* `exposure.include`, so an explicit
  `include=env,configprops` with `show-values=always` slipped past it — a
  real gap, found by reading `check()` and since fixed. The same applies
  to this skill: any claim here about what a rule's `check()` does can go
  stale, so verify it against the current code before acting on it.
